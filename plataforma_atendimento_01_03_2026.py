...
# Conexão com Supabase (via Session Pooler) - VERSÃO PARA NUVEM
def conectar_banco():
    """Conecta ao Supabase usando Session Pooler"""
    try:
        # Para nuvem: usa secrets do Streamlit
        if "DB_URL" in st.secrets:
            ...
            # Para teste local - usa parâmetros diretos
            return psycopg2.connect(
                host="aws-1-eu-west-2.pooler.supabase.com",
                port="5432", 
                database="postgres",
                user="postgres.opuyirrrrzibpxzkrikk",
                password="Humildade15"
            )
