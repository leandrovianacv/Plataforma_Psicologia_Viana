...
def conectar_banco():
    return psycopg2.connect(
        host="localhost",
        port="1987",
        database="Consultas_Psicologia", 
        user="postgres",
        password="Educacao@2025"
    )
        ...
        if pacientes_df.empty:
            ...
            with st.form("form_consulta", clear_on_submit=True):
                ...
                if st.form_submit_button("📅 Agendar Consulta"):
                    # CORREÇÃO APLICADA: Converter numpy.int64 para int
        ...
        if not pacientes_df.empty:
            ...
            with col2:
                st.metric("Cadastros este Mês", 
                         len(pacientes_df[pd.to_datetime(pacientes_df['data_cadastro']) >= pd.Timestamp.now().replace(day=1)]))
        else:
        ...
        if not agenda_df.empty:
            for _, row in agenda_df.iterrows():
                with st.container():
                    ...
                    with col4:
                        st.write(f"**{row['valor_consulta']:,.0f} CVE**")
                    st.divider()
    ...
    try:
        ...
        if not consultas_df.empty:
            consulta_selecionada = st.selectbox(
                "Selecionar Consulta:", 
                consultas_df['nome_completo'] + " - " + consultas_df['data_consulta'].dt.strftime('%d/%m %H:%M')
            )

            col1, col2 = st.columns(2)
            with col1:
                if st.button("✅ Marcar como Realizada", type="primary"):
                    consulta_idx = consultas_df['nome_completo'] + " - " + consultas_df['data_consulta'].dt.strftime('%d/%m %H:%M')
                    consulta_id = consultas_df[consulta_idx == consulta_selecionada].iloc[0]['id']
                    # CORREÇÃO APLICADA: Converter numpy.int64 para int
    ...
    try:
        ...
        with col4:
            taxa_falta = pd.read_sql("""
                ...
                WHERE EXTRACT(MONTH FROM data_consulta) = EXTRACT(MONTH FROM NOW())
            """, conn)
            st.metric("Taxa de Faltas (%)", f"{converter_numpy_para_python(taxa_falta.iloc[0]['taxa']) or 0}")
...
# CORREÇÃO 2: Configuração para domínio personalizado
st.sidebar.markdown("---")
st.sidebar.markdown("### 🌐 Configuração de Domínio")
if st.sidebar.button("🔄 Configurar www.atendimentoviana.cv"):
    st.sidebar.info(
        "**Para usar www.atendimentoviana.cv:**\n\n"
        "1. **Localmente:** Edite o arquivo hosts\n"
        "   - Windows: `C:\\\\Windows\\\\System32\\\\drivers\\\\etc\\\\hosts`\n"
        "   - Adicione: `127.0.0.1 www.atendimentoviana.cv`\n\n"
        "2. **Produção:** Configure DNS apontando para seu servidor"
    )

    # Criar arquivo de configuração para domínio
    config_content = """[server]
port = 8501
address = "0.0.0.0"

[browser]
serverAddress = "www.atendimentoviana.cv"
...
"""
    ...
    try:
        ...
        st.sidebar.success("✅ Configuração de domínio aplicada!")
        st.sidebar.info("🔄 Reinicie o servidor para aplicar as mudanças")
    except Exception as e:
